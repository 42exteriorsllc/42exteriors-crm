...
export default function CRM() {
  ...
  // â”€â”€â”€ Fetch leads from Supabase â”€â”€â”€
  useEffect(() => {
    fetchLeads();
  }, []);
  ...
  const saveForm = async () => {
    ...
    if (editLead) {
      const { data, error } = await supabase
        .from("leads")
        .update({ ...form, estimated_value: Number(form.estimated_value) || 0 })
        .eq("id", editLead)
        .select()
        .single();
      if (!error) {
        setLeads(ls => ls.map(l => l.id === editLead ? data : l));
        if (detailLead?.id === editLead) setDetailLead(data);
      }
    } else {
      const { data, error } = await supabase
        .from("leads")
        .insert([{ ...form, estimated_value: Number(form.estimated_value) || 0 }])
        .select()
        .single();
  ...
  const s = {
    ...
    logoBadge: { fontSize: 9, letterSpacing: 2, color: COLORS.mist, textTransform: "uppercase", paddingLeft: 12, paddingTop: 4 },
    ...
    btnGhost: { background: "transparent", color: COLORS.silver, border: `1px solid ${COLORS.border}`, padding: "7px 16px", fontSize: 11, fontWeight: 400, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer" },
    ...
    card: (div) => ({ background: COLORS.graphite, border: `1px solid ${COLORS.border}`, borderLeft: `2px solid ${divisionColor(div)}`, padding: "12px 14px", cursor: "pointer", transition: "all 0.2s", position: "relative" }),
    ...
    modalClose: { background: "none", border: "none", color: COLORS.mist, fontSize: 20, cursor: "pointer", lineHeight: 1 },
    ...
    panelField: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 },
    ...
    loadingDot: { width: 8, height: 8, borderRadius: "50%", background: COLORS.gold, animation: "pulse 1s infinite" },
  };
  ...
  if (loading) return (
    <div style={{ ...s.app, ...s.loadingScreen }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 24 }}>
        {[0, 1, 2].map(i => (
          <div key={i} style={{ ...s.loadingDot, animationDelay: `${i * 0.2}s`, opacity: 0.6 }} />
        ))}
      </div>
  ...
  if (error) return (
    <div style={{ ...s.app, ...s.loadingScreen }}>
      <div style={{ color: COLORS.red, fontSize: 13, letterSpacing: 1 }}>Connection error: {error}</div>
      <button style={s.btnGold} onClick={fetchLeads}>Retry</button>
    </div>
  );
    ...
    <div style={s.app}>
      ...
      <div style={s.topbar}>
        ...
        <div style={s.topActions}>
          <button style={s.btnGhost} onClick={() => setView(v => v === "kanban" ? "list" : "kanban")}>
      ...
      <div style={s.statsRow}>
        <div style={s.stat}>
          ...
          <div style={s.statSub}>{filtered.length} shown</div>
        </div>
        <div style={s.stat}>
          <div style={s.statLabel}>Pipeline Value</div>
          <div style={s.statValue}>${(pipelineValue / 1000).toFixed(0)}k</div>
          <div style={s.statSub}>Active opportunities</div>
        </div>
        <div style={s.stat}>
          <div style={s.statLabel}>Closed Won</div>
          <div style={s.statValue}>${(wonValue / 1000).toFixed(0)}k</div>
          <div style={s.statSub}>{wonCount} jobs</div>
        </div>
        <div style={s.stat}>
          <div style={s.statLabel}>Close Rate</div>
          <div style={s.statValue}>{leads.length ? Math.round((wonCount / leads.length) * 100) : 0}%</div>
            ...
            return (
              <div key={stage.id} style={s.col(dragOver === stage.id)} onDragOver={e => onDragOver(e, stage.id)} onDrop={e => onDrop(e, stage.id)} onDragLeave={() => setDragOver(null)}>
                <div style={s.colHeader}>
                  <div>
                    <div style={s.colTitle(stage.color)}>{stage.label}</div>
                    {val > 0 && <div style={s.colValue}>{fmt(val)}</div>}
                  </div>
      ...
      {view === "list" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "0 20px 20px" }}>
          <table style={s.table}>
            <thead>
              <tr>{["Name", "Location", "Division", "Source", "Rep", "Value", "Stage", ""].map(h => <th key={h} style={s.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.map(lead => (
                <tr key={lead.id} style={{ cursor: "pointer" }} onClick={() => setDetailLead(lead)} onMouseEnter={e => e.currentTarget.style.background = COLORS.graphite} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  ...
                  <td style={s.td}><span style={{ fontSize: 12, color: COLORS.silver }}>{lead.city}, {lead.state}</span></td>
                  ...
                  <td style={s.td}><span style={{ fontSize: 12, color: COLORS.silver }}>{lead.rep_initials || "â€”"}</span></td>
                  <td style={s.td}><span style={{ fontSize: 13, color: COLORS.gold, fontFamily: "Georgia, serif" }}>{lead.estimated_value ? fmt(lead.estimated_value) : "â€”"}</span></td>
      ...
      {detailLead && (
        <div style={s.panel}>
          <div style={s.panelHeader}>
            <div>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 8 }}>
                ...
                {detailLead.estimated_value ? <span style={{ ...s.tag(COLORS.gold), fontFamily: "Georgia, serif", fontSize: 11 }}>{fmt(detailLead.estimated_value)}</span> : null}
              </div>
            </div>
          ...
          <div style={{ padding: "16px 24px", display: "flex", gap: 10 }}>
            ...
            <button style={{ ...s.btnGhost, color: COLORS.red, borderColor: COLORS.red }} onClick={() => deleteLead(detailLead.id)}>Delete</button>
          </div>
        </div>
      )}
            ...
            <div style={s.modalBody}>
              <div style={s.formRow}>
                <div style={s.formGroup}>
                  ...
                  <input style={s.formInput} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="John Smith" />
                </div>
                <div style={s.formGroup}>
                  <label style={s.formLabel}>Phone</label>
                  <input style={s.formInput} value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="(555) 000-0000" />
                </div>
              </div>
              <div style={s.formRow}>
                <div style={s.formGroup}>
                  <label style={s.formLabel}>Email</label>
                  <input style={s.formInput} value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="email@example.com" />
                </div>
                <div style={s.formGroup}>
                  <label style={s.formLabel}>Est. Value ($)</label>
                  <input style={s.formInput} type="number" value={form.estimated_value} onChange={e => setForm(f => ({ ...f, estimated_value: e.target.value }))} placeholder="12000" />
                </div>
              </div>
              <div style={s.formGroup}>
                <label style={s.formLabel}>Address</label>
                <input style={s.formInput} value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} placeholder="123 Main St" />
              </div>
              <div style={s.formRow}>
                <div style={s.formGroup}>
                  <label style={s.formLabel}>City</label>
                  <input style={s.formInput} value={form.city} onChange={e => setForm(f => ({ ...f, city: e.target.value }))} placeholder="Cherry Hill" />
                </div>
                <div style={s.formGroup}>
                  <label style={s.formLabel}>State</label>
                  <select style={s.formSelect} value={form.state} onChange={e => setForm(f => ({ ...f, state: e.target.value }))}>
                    {STATES.map(st => <option key={st}>{st}</option>)}
                  </select>
                </div>
              </div>
              <div style={s.formRow}>
                <div style={s.formGroup}>
                  ...
                  <select style={s.formSelect} value={form.division} onChange={e => setForm(f => ({ ...f, division: e.target.value }))}>
                    {DIVISIONS.map(d => <option key={d}>{d}</option>)}
                  </select>
                </div>
                <div style={s.formGroup}>
                  <label style={s.formLabel}>Lead Source</label>
                  <select style={s.formSelect} value={form.source} onChange={e => setForm(f => ({ ...f, source: e.target.value }))}>
              ...
              <div style={s.formRow}>
                <div style={s.formGroup}>
                  ...
                  <select style={s.formSelect} value={form.stage} onChange={e => setForm(f => ({ ...f, stage: e.target.value }))}>
                    {STAGES.map(st => <option key={st.id} value={st.id}>{st.label}</option>)}
                  </select>
                </div>
                <div style={s.formGroup}>
                  <label style={s.formLabel}>Sales Rep</label>
                  <input style={s.formInput} value={form.rep_initials} onChange={e => setForm(f => ({ ...f, rep_initials: e.target.value }))} placeholder="Initials or name" />
                </div>
              </div>
              {form.division === "Insurance" && (
                <div style={s.formRow}>
                  <div style={s.formGroup}>
                    <label style={s.formLabel}>Insurance Carrier</label>
                    <input style={s.formInput} value={form.carrier} onChange={e => setForm(f => ({ ...f, carrier: e.target.value }))} placeholder="State Farm" />
                  </div>
                  <div style={s.formGroup}>
                    <label style={s.formLabel}>Claim Number</label>
                    <input style={s.formInput} value={form.claim_number} onChange={e => setForm(f => ({ ...f, claim_number: e.target.value }))} placeholder="CLM-00000" />
                  </div>
                </div>
              )}
              <div style={s.formGroup}>
                <label style={s.formLabel}>Notes</label>
                <textarea style={s.formTextarea} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="Damage description, adjuster details, next steps..." />
              </div>
            </div>
